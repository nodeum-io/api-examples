...
def get_nas_pool_id(name, type):
    url = "{host}/api/backend/nas/pools".format(host=my_host)
    resp = requests.get(url, auth=my_auth)
    res = resp.json()
    if res['code'] == 200:
        for pool in res['pools']:
            if pool['name'] == name and pool['type'] == type:
                return pool['id']

        raise Exception("Could not find pool name {}".format(name))
    else:
        raise Exception(res)


def create_task(source_path, nas_pool_destination, task_name):
    task = {
        "rule": {
            "name": task_name,
            "comment": "",
...
nas_pool_id = get_nas_pool_id("smb-offline", OFFLINE)
print("Nas pool id is {}".format(nas_pool_id))

task_id = create_task("/nfs-primary/bamfiles", nas_pool_id, "Workshop")
